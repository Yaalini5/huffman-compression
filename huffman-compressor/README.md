...
## Web app (upload and download)

This repository now includes a Flask web interface where users can:

- Upload a file and compress it into `.abiz`
- Upload a `.abiz` file and decompress it back
- Download the processed output directly from the browser

### Run locally

1. Install Python 3.10+ and a C++ compiler with `g++` in PATH.
...
The app auto-builds the C++ binary (`huffman-web` / `huffman-web.exe`) on first request.
...
Make sure your deploy environment has a C++ compiler available so the Huffman binary can be built.
...
```bash
docker build -t huffman-compressor:latest .
```
...
This image installs a minimal build toolchain, compiles the C++ binary at image build time, installs Python dependencies, and serves the Flask app with `gunicorn`.
